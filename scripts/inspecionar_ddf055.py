#!/usr/bin/env python3
"""Captura um mapa detalhado da aba Dedução/DDF055 aberta no Chrome."""
...
DEFAULT_OUT_DIR = Path.home() / "Documents" / "AutoLiquid" / "falhas-automacao" / "ddf055-dom"
...
() => {
  ...
  const nearbyText = (el) => {
    const root = el.closest(".row, tr, .form-group, .control-group, .col-md-6, .col-sm-6, .modal-body, .tab-pane")
      || el.parentElement;
    return clean(root?.innerText || root?.textContent || "").slice(0, 520);
  };
  const prefixes = [
    "sfdeducao", "sfdeducaocodsit", "sfdeducaocodugpgto", "sfdeducaodtvenc",
    "sfdeducaodtpgtoreceb", "sfdeducaovlr", "sfdeducaopossui_acrescimo",
    "sfdeducaoconfirma_dados", "confirma-dados-deducao-", "novo-recolhedor",
    "recolhedor", "vlrbasecalculo", "vlrPrincipal", "txtinscra", "txtinscrb",
    "sfpredoc", "municipioDeducaoPreDoc", "txtprocesso", "obser"
  ];
  const isDeducaoField = (el) => {
    const id = String(el.id || "");
    const name = String(el.getAttribute("name") || "");
    return prefixes.some((prefix) => id.startsWith(prefix) || name.startsWith(prefix));
  };
  const fields = Array.from(document.querySelectorAll("input, select, textarea"))
    .filter((el) => isDeducaoField(el) || nearbyText(el).toLowerCase().includes("ddf055"))
    .map((el, index) => ({
      index,
      ...
      cssPath: cssPath(el),
      onchange: String(el.getAttribute("onchange") || ""),
      onkeyup: String(el.getAttribute("onkeyup") || ""),
      nearbyText: nearbyText(el),
      options: optionsOf(el),
      select2Text: el.id
        ? clean(document.getElementById(`select2-${el.id}-container`)?.textContent || "")
        : "",
    }));
  const buttons = Array.from(document.querySelectorAll("button, a, input[type='button'], input[type='submit']"))
    .filter((el) => {
      ...
      return visible(el) && (
        ...
        || id.startsWith("novo-recolhedor")
        || /confirmar|dedu[cç][aã]o|pr[eé]-?doc|\+|ddf055/i.test(text)
      );
    })
    .map((el, index) => ({
      index,
      ...
      onclick: String(el.getAttribute("onclick") || "").slice(0, 400),
      disabled: !!el.disabled,
      rect: rectOf(el),
      cssPath: cssPath(el),
      nearbyText: nearbyText(el),
  ...
  for (const did of deducaoIds) {
    ...
    const aba = document.getElementById(`aba-deducao${did}`);
    const confirmar = document.getElementById(`confirma-dados-deducao-${did}`);
    byDeducaoId[did] = {
      did,
      situacaoValue: situacao ? String(situacao.value || "") : "",
      situacaoText: situacao?.selectedIndex >= 0 ? clean(situacao.options[situacao.selectedIndex]?.textContent || "") : "",
      select2Text: clean(document.getElementById(`select2-sfdeducaocodsit${did}-container`)?.textContent || ""),
      abaText: clean(aba?.textContent || ""),
      abaDataCodsit: String(aba?.getAttribute("data-codsit") || ""),
      painelIds: Array.from(document.querySelectorAll(`[id$="_${did}"]`)).map((el) => el.id),
      confirmado: String(document.getElementById(`sfdeducaoconfirma_dados${did}`)?.value || ""),
      confirmarVisible: visible(confirmar),
      fields: fields.filter((field) => String(field.id || field.name || "").endsWith(did)),
    };
  }
  const activePanel = Array.from(document.querySelectorAll(".tab-pane.active, .tab-pane.show, [role='tabpanel']"))
    .map((el) => ({ id: el.id || "", text: clean(el.innerText || el.textContent).slice(0, 1200), visible: visible(el) }))
    .filter((item) => item.visible);
  const notices = Array.from(document.querySelectorAll(".swal2-container, .swal2-popup, .pnotify, .ui-pnotify, .sweet-alert, .jconfirm"))
  ...
  return {
    ...
    activePanel,
    notices,
    deducaoHtmlLength: document.getElementById("deducao")?.innerHTML?.length || 0,
  };
}
"""
...
def capture_ddf055_snapshot(page: Any, out_dir: str | Path = DEFAULT_OUT_DIR, prefix: str = "") -> dict[str, Any]:
    """Captura JSON focado, HTML e screenshot da pagina atual de Dedução/DDF055."""
    ...
    base = output_dir / f"{file_prefix}ddf055-dom-{timestamp}"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Inspeciona a aba Dedução/DDF055 aberta no Chrome.")
    ...
    try:
        snapshot = capture_ddf055_snapshot(page, args.out_dir, args.prefix)
